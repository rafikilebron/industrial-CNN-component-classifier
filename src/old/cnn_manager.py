...
class CNN_ResNet18(nn.Module):
    ...
    def _init_model_architecture(self):
        """
        Initialize the CNN architecture (ResNet18) by trying to download the pretrained weights.
    ...
    def _init_hyperparameters(self):
        """
        Initialize the model hyperparameters (used for training).
    ...
    def _init_transforms(self):
        """
        Initialize both training and validation transforms. Used for dataset creation.
    ...
    def update_n_classes(self, classes_list):
        """
        Updates the number of classes in the dataset.

        Args:
            classes_list (list): A list of classes.
    ...
    def create_dataset(self, root, output_dir, train_ratio=0.9):
        """
        Creates the model's train and validation datasets given an images directory.
        Stores both datasets in object's parameters.

        Args:
            root (str):             Root directory of the image dataset.
            output_dir (str):       Directory where split folders (train/val) will be created.
            train_ratio (float):    Ratio of training dataset to validation dataset.
    ...
    def transform_image(self, image_path):
        """
        Converts the input image to tensor given the file path.

        Args:
            image_path (str): File path to image.

        Returns:
            input_tensor (torch.Tensor): Tensor containing image data.
    ...
    def _train(self, print_=True):
        """
        Train the CNN model and returns the validation accuracy and the training loss during training.

        Args:
            print_ (bool): If true, prints learning rate, validation loss and validation accuracy on the current epoch.

        Returns:
            accuracy_list (list): A list of the validation accuracy, ready to plot.
            loss_list (list): A list of the validation loss, ready to plot.
        ...
        accuracy_best = 0  # Tracks best validation accuracy
    ...
    def predict(self, image_path, classes, print_=True):
        """
        Performs a prediction given an input image path.

        Args:
            image_path (str): File path to image.
            classes (list): A list of classes to predict.
            print_ (bool): If true, prints the file name and prediction.

        Returns:
            prediction (int): Predicted class index.
            predicted_classes (str): Predicted class name.
    ...
    def save_model(self, path, name='model.pt'):
        ...
        Args:
            path (str): Output path to save model.
            name (str): Name of the .pt file.
    ...
    def load_model(self, path):
        """
        Loads the model from .pt file and stores it as self.model.
    ...
    def print_hypams(self):
        """
        Prints the current hyperparameters.
